...
class JiraAPI(object):
    ...
    def send(self, method, path, message):
        conn = httplib.HTTPSConnection(self.host)
        conn.request(method,
                     '%s/%s?os_username=%s&os_password=%s' % (self.api_path, path, self.username, self.password),
                     json.dumps(message), {'Content-type': 'application/json'})
    ...
    def default_api(cls):
        ...
        return cls(conf.get('jira_default', 'host'), conf.get('jira_default', 'path'),
                   conf.get('jira_default', 'username'), conf.get('jira_default', 'password'))
...
class Worklog(object):
    ...
    @classmethod
    def get_by_issue(cls, key):
        response = cls.api().get('issue/%s/worklog' % key, {'fields': 'body'})
        if response.status == 200:
            worklogs = json.load(response)['worklogs']
            return [cls(key, node) for node in worklogs]
        else:
            raise APIException('could not get worklog for %s' % key, response)
    ...
    @classmethod
    def get(cls, issue_key, id):
        response = cls.api().get('issue/%s/worklog/%s' % (issue_key, id), {'fields': 'body'})
        if response.status == 200:
            return cls(issue_key, json.load(response))
        else:
            raise APIException('could not get comment %s/%s' % (issue_key, id), response)
    ...
    @classmethod
    def add(cls, issue_key, comment):
        response = cls.api().send('POST', 'issue/%s/worklog' % issue_key, {"comment": comment, "timeSpentSeconds" : 12000})
        if response.status != 201: raise APIException('could not add worklog', response)
    ...
    def update(self, comment):
        response = self.api().send('PUT', 'issue/%s/worklog/%s' % (self.issue_key, self.id), {"comment": comment})
        if response.status != 200: raise APIException('could not update worklog %s/%s' % (key, id), response)
    ...
    #
    def delete(self):
        response = self.api().send('DELETE', 'issue/%s/worklog/%s' % (self.issue_key, self.id), {})
        if response.status != 204: raise APIException('could not delete worklog %s/%s' % (self.issue_key, id), response)
